...
public class PostService {
    ...
    public PageBean<Post> listPostByHeat(int curPage){////////
        //每页记录数，从哪开始
        int limit = 8;
        ...
        //分页得到数据列表
        List<Post> postList = postMapper.listPostByHeat(offset,limit);
    ...
    public List<Post> listAllPost(){
        List<Post> postList=postMapper.listAllPost();
        return postList;
    }
    public void setTopPost(int pid){
        postMapper.setTopPost(pid);
    }
    public int returnTop(){
        return postMapper.returnTop();
    }
    //按时间列出帖子
    public PageBean<Post> listPostByTime(int curPage) {
        ...
        if(postMapper.returnTop()!=0){//置顶
            Post post=postMapper.getPostByPid(postMapper.returnTop());
            postList.set(0,post);
        }
        Jedis jedis = jedisPool.getResource();
        for(Post post : postList){
    ...
    public void editEssay(@Param("pid") int pid, @Param("content") String content){
        postMapper.editEssay(pid,content);
    }
}
